...
pmark_phase_setting = 360; % this is set in ard. to make 4us delay occur at 0phase
...
hold on
plot(x,180*y1(:,segs)./max(max(y1)),'color',col(1,:));
plot(x,rad2deg(y1_pha(:,segs)),'color',col(1,:));
plot(x,180*y2(:,segs)./max(max(y2)),'color',col(2,:));
plot(x,180*y3(:,segs)./max(max(y3)),'color',col(3,:));
% plot(x,y4(:,segs)./max(max(y4)),'color',col(4,:));
